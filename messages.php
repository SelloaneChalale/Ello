...
if(isset($_POST['send-msg'])){
    if (!empty($_POST['message'])) {
        ...
        $sql = "INSERT INTO messages (msg_id, sender_id, receiver_id, msg) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param('ssss', $gen, $uid, $id, $message);
        ...
        $errors['input_error'] = "Message field cannot be empty";
...
<body class="messenger">
                        ...
                        <div id="message-frame">
                            <div class="message-sidepanel">
                                <div class="message-profile">
                                    <div class="wrap">
                                        <img src="assets/images/users/user-4.png" class="online conv-img" alt="Conversation user" />
                                        <p>Arthur Minasyan</p>
                            ...
                            <div class="content">
                                <div class="row">
                                    <div class="col-md-12 messenger-top-section">
                                        <div class="contact-profile d-flex align-items-center justify-content-between">
                                            <div class="messenger-top-luser df-aic">
                                                ...
                                                <a href="#" class="message-profile-name"><?php echo $username;?></a>
                                            </div>
                                    ...
                                    <?php 
                         $emailQuery = "SELECT * FROM messages WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?)";
    ...
    while ($row = $result->fetch_assoc()) {
        $message = htmlspecialchars($row['msg']); // Avoid XSS by escaping the message
        $sender_id = $row['sender_id'];
        $receiver_id = $row['receiver_id'];
         // Determine if the current user is the sender or receiver
         $messageClass = ($sender_id == $uid) ? 'sender' : 'receiver';
        // $timestamp = $row['timestamp']; // Assuming you have a timestamp column in your messages table
    ?>
    ...
    <div class="input-group messenger-input">
        <input type="text" class="form-control search-input" name="message" placeholder="Type a message..." aria-label="Type a message..." aria-describedby="button-addon2">
        <button type="submit" name="send-msg" class="btn btn-primary">Send</button>
    </div>
</form>
