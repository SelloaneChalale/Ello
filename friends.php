...
// $mk ="SELECT * FROM users WHERE id!='$uid' ORDER BY RAND() ";
// $mk ="SELECT * FROM users JOIN block_list ON  WHERE blocker_id!='$uid' ORDER BY RAND() ";
$sql = "SELECT u.* FROM users u 
        LEFT JOIN block_list b1 ON u.id = b1.blocked_id AND b1.blocker_id = '$uid'
        LEFT JOIN block_list b2 ON u.id = b2.blocker_id AND b2.blocked_id = '$uid'
        WHERE b1.blocked_id IS NULL AND b2.blocker_id IS NULL AND id!='$uid' ";
                                ...
                                <div class="profile-info-left">
                                    <div class="text-center">
                                        <div class="profile-img w-shadow">
                                            ...
                                            <img src="assets/images/users/user-4.png" alt="Avatar" class="avatar img-circle">
                                        ...
                                        <p class="profile-fullname mt-3"><?php echo $fname;?></p>
                                        <p class="profile-username mb-3 text-muted"><?php echo $_SESSION['email'];?></p>
                                    </div>
                                                    ...
                                                    <ul class="dropdown-menu notify-drop nav-drop shadow-sm" aria-labelledby="searchDropdown">
                                                        ...
                                                        <div class="drop-content">
                                                            ...
                                                            <li class="dropdown-item">
                                                                ...
                                                                <div class="col-md-10 col-sm-10 col-xs-10">
                                                                    <a href="#" class="notification-user">Ello Social Network <img src="assets/images/theme/verify.png" width="10px" class="verify" alt="Group verified"></a>
                                                                    <a href="#" class="btn btn-quick-link join-group-btn border text-right float-right">
                                                                        Join
                                                                    </a>
                                                ...
                                                <?php foreach($network as $key => $nt):?>
                                                    <div class="col-md-4 col-sm-6">
                                                        <div class="card group-card shadow-sm">
                                                            ...
                                                            <div class="card-body">
                                                                <h5 class="card-title"><?php echo $nt['username'];?> <img src="assets/images/theme/verify.png" width="10px" class="verify" alt="Group verified"></h5>
                                                                <p class="card-text">10k Members 20+ post a week</p>
                                                                
                                                                <?php 

/////////////////////-===============///////CHECKING IF USER ACCEPTED CONNECTION///-==============///////////////
$id = $nt['id'];
$emailQuery = "SELECT * FROM network WHERE (sender_id=? AND receiver_id=? AND status=1) OR (sender_id=? AND receiver_id=? AND status=1) LIMIT 1";
$stmt = $conn->prepare($emailQuery);
$stmt->bind_param('ssss', $id,$uid,$uid,$id);
$stmt->execute();
$result = $stmt->get_result();
$accept = $result->num_rows;
$stmt->close();
//////////////////-============================///////end//=----------------------------------///////////

//////////////////////=--------------------------//////CHECKING IF USER HAS NO RESPONDED//-=====================/////////////
$id = $nt['id'];
$emailQuery = "SELECT * FROM network WHERE receiver_id=? AND sender_id=? AND status='0' LIMIT 1";
$stmt = $conn->prepare($emailQuery);
$stmt->bind_param('ss', $id,$uid);
$stmt->execute();
$result = $stmt->get_result();
$noResponse = $result->num_rows;
$stmt->close();
////////////////////////////////////=---------------------------------///END//-===========////////////////////


 ////////////////////////////CHECKING IF USERS ARE CONNECTED OR NOT//////////////////////////////////
 $id = $nt['id'];
 $emailQuery = "SELECT * FROM network WHERE (sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?) LIMIT 1";
 $stmt = $conn->prepare($emailQuery);
 $stmt->bind_param('ssss', $id,$uid,$uid,$id);
 $stmt->execute();
 $result = $stmt->get_result();
 $notConnected = $result->num_rows;
 $stmt->close();
 //////////////////////////////////////=---------------------/END=================================//////////////////////
    ...
    $stmt->close();
///////////////////////////END/////////
$sql = "SELECT network.sn, users.username FROM network INNER JOIN users ON network.receiver_id = users.id WHERE network.receiver_id = $uid AND network.status = 1";
$result = $conn->query($sql);
?>
                                                                <div class="btn-group w-100" role="group">
                                                                <!-- <a href="./messages.php" class="btn btn-quick-link join-group-btn border w-100"> -->
                                                                            <?php  if($accept > 0){
                                                                echo "<a href='messages?id=$id' class='btn btn-quick-link join-group-btn border w-100'>Message</a>";
                                                                        }
																		
																		?>
                                                                        <?php  if($noResponse > 0){
                                                                echo "<class='btn btn-quick-link join-group-btn border w-100'> Pending";
                                                                        }?>
                                                                        <?php
																															
																		if (empty($notConnected)) {
																			echo "<a href='connect?id=$id' class='btn btn-quick-link join-group-btn border w-100'>Connect</a>";
																		}
																		 if($row > 0) {
																			
														echo "<a href='accept?id=" . $row["sn"] . "'><class='btn btn-quick-link join-group-btn border w-100'>Accept</a>    |      <a href='reject?id=" . $row["sn"] . "'><class='btn btn-quick-link join-group-btn border w-100'>Reject</a>";
																		}
																		?>
                                                                    ...
                                                                    <div class="btn-group" role="group">
                                                                        ...
                                                                        <div class="dropdown-menu" aria-labelledby="friendsMore">
                                                                            ...
                                                                            <a class="dropdown-item" href="block?id=<?php echo $id;?>">Block</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
